...
def list_bucket_files(storage: Storage, bucket: str, prefix: str = "") -> list[tuple[str, int]]:
    """List all files in bucket with sizes using S3-compatible API for performance."""
    if storage.config.protocol != "s3":
...
def get_bucket_stats(storage: Storage, bucket: str, prefix: str = "") -> tuple[int, int]:
    """Get bucket file count and total size using S3-compatible API."""
    if storage.config.protocol != "s3":
