...
from grin_transfer.docker.validation import (
...
class TestProcessLocalStoragePath:
    ...
    @patch("grin_transfer.docker.validation.is_docker_environment", return_value=False)
    ...
    @patch("grin_transfer.docker.validation.is_docker_environment", return_value=True)
    @patch("pathlib.Path.mkdir")
    @patch("pathlib.Path.unlink")
