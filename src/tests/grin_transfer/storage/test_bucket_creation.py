...
from grin_transfer.sync.utils import ensure_bucket_exists, reset_bucket_cache


@pytest.fixture(autouse=True)
def clean_aws_env(monkeypatch):
    """Remove AWS environment variables for each test.

    Bucket creation tests use moto's mock_aws() and shouldn't pick up
    real AWS credentials from the environment.
    """
    ...
    async def test_bucket_creation_r2(self):
        ...
        with patch.dict(os.environ, {"MOTO_S3_CUSTOM_ENDPOINTS": custom_endpoint}):
            with mock_aws():
                ...
                with patch(
                    "grin_transfer.sync.utils.load_r2_credentials", return_value=("test_access_key", "test_secret_key")
                ):
                    # Don't create bucket beforehand - test should create it
