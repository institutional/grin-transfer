...
from grin_transfer.constants import LOCAL_STORAGE_DEFAULTS
from grin_transfer.storage.factories import create_local_storage_directories
...
class TestCreateLocalStorageDirectories:
    ...
    async def test_create_directories_non_docker(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("grin_transfer.docker.validation.is_docker_environment", return_value=False):
    ...
    async def test_create_directories_docker_valid_path(self):
        ...
        with patch("grin_transfer.docker.validation.is_docker_environment", return_value=True):
            ...
            with patch("pathlib.Path.mkdir"):
                with patch("pathlib.Path.write_text"):
                    with patch("pathlib.Path.unlink"):
                        ...
                        with patch(
                            "grin_transfer.docker.validation.translate_docker_data_path_for_local_storage",
    ...
    async def test_create_directories_docker_invalid_path(self):
        ...
        with patch("grin_transfer.docker.validation.is_docker_environment", return_value=True):
            with patch(
                "grin_transfer.docker.validation.process_local_storage_path",
                side_effect=ValueError("Invalid Docker path"),
            ):
