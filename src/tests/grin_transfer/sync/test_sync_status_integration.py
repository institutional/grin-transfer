...
from grin_transfer.collect_books.models import BookRecord
...
@pytest.mark.asyncio
async def test_get_books_for_sync_with_converted_status(db_tracker, temp_db):
...
@pytest.mark.asyncio
async def test_get_books_for_sync_with_converted_filter(db_tracker, temp_db):
    ...
    sync_books = await get_barcodes_from_set_for_testing(db_tracker, converted_barcodes, limit=10)

    # Only books in the converted_barcodes set should be returned
    sync_barcodes = set(sync_books)
    assert sync_barcodes == converted_barcodes


@pytest.mark.asyncio
async def test_get_books_for_sync_excludes_non_requested(db_tracker, temp_db):
    ...
    await db_tracker.save_book(book2)
    ...
    assert set(sync_books) == {barcode, barcode2}


@pytest.mark.asyncio
async def test_sync_status_tracking(db_tracker, temp_db):
    ...
    await batch_write_status_updates(temp_db, sync_status_updates)

    # Verify latest sync status
    latest_sync_status = await get_latest_status_for_testing(db_tracker, barcode, "sync")
    assert latest_sync_status == "completed"


@pytest.mark.asyncio
async def test_get_books_for_sync_with_status_filter(db_tracker, temp_db):
    ...
    all_sync_books = await get_barcodes_needing_sync_for_testing(db_tracker, limit=10)

    # Should include books with no sync status or failed status
    expected_all = {"STAT001", "STAT004"}  # NULL or failed
    assert set(all_sync_books) == expected_all

    # Test with specific status filter
    failed_sync_books = await get_barcodes_with_sync_status_for_testing(db_tracker, "failed", limit=10)

    # Should only include books with failed status
    assert set(failed_sync_books) == {"STAT004"}


@pytest.mark.asyncio
async def test_sync_book_eligibility(db_tracker, temp_db):
    ...
    assert barcode in sync_books


@pytest.mark.asyncio
async def test_backwards_compatibility_legacy_status(db_tracker):
...
@pytest.mark.asyncio
async def test_status_change_ordering(db_tracker, temp_db):
