...
from grin_transfer.queue_utils import get_previous_queue_books
...
async def test_previous_queue_basic_filtering(mock_grin_client, db_tracker, temp_db):
    ...
    with patch("grin_transfer.sync.pipeline.get_in_process_set") as mock_in_process:
...
async def test_previous_queue_database_queries(mock_grin_client, db_tracker, temp_db):
    ...
    with patch("grin_transfer.sync.pipeline.get_in_process_set") as mock_in_process:
        ...
        with patch("grin_transfer.sync.pipeline.get_previous_queue_books") as mock_previous_queue:
