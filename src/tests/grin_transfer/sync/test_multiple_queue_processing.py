...
async def test_single_queue_regression(mock_pipeline, mock_task_funcs, task_manager):
    ...
    with (
        patch("grin_transfer.sync.task_manager.commit_book_record_updates") as mock_commit,
        patch("grin_transfer.sync.task_manager.get_updates_for_task") as mock_updates,
        patch("grin_transfer.sync.task_manager.ProcessStageMetrics.determine_book_outcome") as mock_outcome,
    ):
        # Set up mocks
...
async def test_multiple_queues_independent_processing():
    ...
    with patch("grin_transfer.sync.pipeline.get_books_from_queue"):
        with patch("grin_transfer.sync.pipeline.process_books_with_queue"):
            with patch("grin_transfer.sync.pipeline.filter_and_print_barcodes"):
