...
from grin_transfer.sync.progress_reporter import SlidingWindowRateCalculator
from grin_transfer.sync.task_manager import TaskManager, process_books_with_queue
from grin_transfer.sync.tasks.task_types import TaskAction, TaskResult, TaskType
...
class TestQueueBasics:
    ...
    async def test_queue_processes_books_successfully(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase") as mock_download,
            patch("grin_transfer.sync.task_manager.process_processing_phase") as mock_processing,
        ):
...
class TestConcurrentProcessing:
    ...
    async def test_concurrent_processing_eliminates_dead_time(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase", side_effect=track_download_time),
            patch("grin_transfer.sync.task_manager.process_processing_phase", side_effect=track_processing_time),
        ):
    ...
    async def test_bounded_queue_provides_backpressure(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase", slow_download),
            patch("grin_transfer.sync.task_manager.process_processing_phase", slow_processing),
        ):
...
class TestWorkerManagement:
    ...
    async def test_configurable_worker_count(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase", side_effect=track_download_workers),
            patch("grin_transfer.sync.task_manager.process_processing_phase", side_effect=track_processing_workers),
        ):
...
class TestErrorHandling:
    ...
    async def test_individual_book_failures_dont_crash_workers(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase", side_effect=failing_download),
            patch("grin_transfer.sync.task_manager.process_processing_phase", side_effect=normal_processing),
        ):
...
class TestEndToEndIntegration:
    ...
    async def test_database_updates_accumulated(
        ...
        with (
            patch("grin_transfer.sync.task_manager.process_download_phase", side_effect=download_with_db_updates),
            patch("grin_transfer.sync.task_manager.process_processing_phase", side_effect=processing_with_updates),
        ):
            await process_books_with_queue(
                barcodes,
                mock_pipeline,
