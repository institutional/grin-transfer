...
def test_build_storage_config_dict_storage_path_takes_precedence():
    """build_storage_config_dict should use --storage-path when specified."""
    args = MockArgs(
        storage_path="/tmp/preferred-storage",
        storage_config=["endpoint_url=https://example.com"],
        bucket_raw=None,
        bucket_meta=None,
        bucket_full=None,
        storage="local",
    ...
    assert result["base_path"] == "/tmp/preferred-storage"
    assert result["endpoint_url"] == "https://example.com"


def test_build_storage_config_dict_no_storage_path():
    """build_storage_config_dict should work normally when neither --storage-path nor base_path are specified."""
    args = MockArgs(
        storage_path=None,
        storage_config=["endpoint_url=https://s3.amazonaws.com"],
        bucket_raw="test-raw",
    ...
    assert result["bucket_raw"] == "test-raw"
    assert result["bucket_meta"] == "test-meta"
    assert result["bucket_full"] == "test-full"
